...
app.controller('SaleEntryCtrl', function($scope, $http) {
    ...
    $scope.getProfitFn = function(index){
        var total = 0.00;
        total = parseFloat( ($scope.cart[index].sale_price - $scope.cart[index].purchase_price) * $scope.cart[index].quantity );
        $scope.cart[index].profit = total;
	...
	$scope.setSubtotalFn = function(index){
		$scope.cart[index].subtotal = ($scope.cart[index].sale_price * $scope.cart[index].quantity) - $scope.cart[index].discount;
