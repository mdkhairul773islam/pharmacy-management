...
app.controller('returnPurchaseCtrl', function ($scope, $http) {
					...
					if (itemInfo.length > 0) {
						angular.forEach(itemInfo, function (row, index) {
							...
							var item = {
								...
								quantity: (parseFloat(row.quantity) + parseFloat(row.return_quantity)),
								return_quantity: parseFloat(row.return_quantity),
								new_quantity: 0,
	...
	// get new quantity
	$scope.getNewQuantityFn = function (index) {
		var total = 0;
		total = $scope.cart[index].quantity - $scope.cart[index].return_quantity;
		$scope.cart[index].new_quantity = total.toFixed(2);
		return $scope.cart[index].new_quantity;
	};
	...
	$scope.setSubtotalFn = function (index) {
		var total = 0;
		total = $scope.cart[index].price * $scope.cart[index].new_quantity;
