...
class Dashboard extends Admin_controller{
    ...
    public function index() {
        ...
        //Client
        $this->data['total_client'] = $this->action->read_distinct("parties", array(), 'code');
        foreach($this->data['total_client'] as $key => $value){
            if($value->code != null){
                $counter1 += count($value->code);
            }
        }
        $this->data['totalClient'] = $counter1;
        
        //Manufacturer
        $this->data['total_manufacturer'] = $this->action->read_distinct("subcategory", array(), 'subcategory');
        foreach($this->data['total_manufacturer'] as $key => $value){
            if($value->subcategory != null){
                $counter2 += count($value->subcategory);
            }
        }
        $this->data['totalManufacturer'] = $counter2;
        
        //Product
        $this->data['total_product'] = $this->action->readGroupBy("products", 'product_code', array());
        foreach($this->data['total_product'] as $key => $value){
            if($value->product_code != null){
                $counter3 += count($value->product_code);
            }
        }
        $this->data['totalProduct'] = $counter3;
        
        //Invoice
        $this->data['total_invoice'] = $this->action->readGroupBy("saprecords", 'voucher_no', array());
        foreach($this->data['total_invoice'] as $key => $value){
            if($value->voucher_no != null && $value->status=='sale' && $value->trash == 0){
                $counter4 += count($value->voucher_no);
            }
        }
        $this->data['totalInvoice'] = $counter4;
         ...
         * cash = ( Sale Paid + Due Collection + Credit Client Collection ) - (supplier_payment + cost )
         **/ 
       ...
       foreach($supplierDueInfo as $key=>$value){
        ...
        /**
         * chart data collecting from database table
         **/
         
        // bar-chart-sale
        $this->data['sale'] = [];
        $current = date('Y-m-d');
        $previous = date('Y-m-d',strtotime(date('Y-m-d')."-1 month"));
        $month = date("M", strtotime($previous));
        
        $amount = array();
        $month = array();
        
        for ($i =-5; $i<=0; $i++){
            $previous = date('Y-m-d',strtotime(date('Y-m-d')."$i"."month"));
            
            $where = array(
                'month(sap_at)' => date("m", strtotime($previous)), 
                'trash'         => 0,
                'status'        => 'sale'
            );
            $records = $this->action->read_sum('saprecords','total_bill', $where);
            $amount = ($records[0]->total_bill > 0 ) ? (int) $records[0]->total_bill : 0.0;
            
            $month = date("M", strtotime($previous));
            
            $this->data['sale'][$month] = $amount;
            //echo ("month : ".$month.": amount : ".$amount)."</br>";
        }
            
        $this->data['sale'] = json_encode($this->data['sale']);
        ...
        for ($i =-5; $i<=0; $i++){
            ...
            $this->data['purchase'][$month] = $amount;
            //echo ("month : ".$month.": amount : ".$amount)."</br>";
        }
            
        $this->data['purchase'] = json_encode($this->data['purchase']);
        ...
        for ($i =-5; $i<=0; $i++){
            ...
            $where = array(
                'month(date)' => date("m", strtotime($previous)), 
                'trash'     => 0,
            );
            $records = $this->action->read_sum('cost','amount', $where);
            $amount = ($records[0]->amount > 0 ) ? (int) $records[0]->amount : 0.0;
            
            $month = date("M", strtotime($previous));
            
            $this->data['cost'][$month] = $amount;
            //echo ("month : ".$month.": amount : ".$amount)."</br>";
        }
        
        $this->data['cost'] = json_encode($this->data['cost']);
