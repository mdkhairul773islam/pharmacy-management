...
class Report extends Admin_Controller {
    ...
    public function Profit_loss_report(){
        ...
        $con = array('sap_at' => date("Y-m-d"), 'status' => 'sale', 'trash' => 0);
        $this->data['resultInfo']=$this->action->read('sapitems',$con);
    ...
    public function Profit_report(){
        $this->data['meta_title'] = 'Profit Report';
        $this->data['active']     = 'data-target="report_menu"';
        $this->data['profitInfo'] = null;
        
        $where = array("trash"  => 0);
        $where1 = array("trash"  => 0);
        $con = array('sap_at' => date("Y-m-d"), 'trash' => 0);
        /*$this->data['profitInfo']=$this->action->readGroupBy('saprecords','voucher_no',$con);
        $this->data['costInfo']=$this->action->read('cost',$where);*/
        ...
        if(isset($_POST['show'])){
            ...
            if(isset($_POST['date'])){
                foreach ($_POST['date'] as $key => $value) {
                    if($value != NULL && $key == "from"){$where['sap_at >='] = $value;}
                    if($value != NULL && $key == "to"){$where['sap_at <='] = $value;}
                    if($value != NULL && $key == "from"){$where1['date >='] = $value;}
                    if($value != NULL && $key == "to"){$where1['date <='] = $value;}
                }
            }
            $this->data['profitInfo']=$this->action->readGroupBy('saprecords','voucher_no',$where);
            $this->data['costInfo']=$this->action->read('cost', $where1, 'desc');
        }
        $this->load->view($this->data['privilege'] . '/includes/header', $this->data);
        $this->load->view($this->data['privilege'] . '/includes/aside', $this->data);
        $this->load->view($this->data['privilege'] . '/includes/headermenu', $this->data);
        $this->load->view('components/report/productProfit', $this->data);
        $this->load->view($this->data['privilege'] . '/includes/footer', $this->data);
    }
}
